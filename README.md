...
| **sentinel-dashboard** | sentinel控制台<br />适配Nacos持久化改造<br />publish规则到Nacos | 从官方1.8.8中迁移出<br />可`mvn clean package -DskipTests`构建 |
...
规则放在应用的`degrade-rules.json`文件中，通过资源前缀区分不同类型，如feign，redis等：

```json
[
    {
        "resource": "GET#/users/detail", 				// 网关对当前服务降级
        "grade": 1,
        "count": 0.5,
        "timeWindow": 10,
        "minRequestAmount": 5,
    ...
    {
        "resource": "redis#value#get#users:{userId}",   // 当前服务对redis降级
        "grade": 2, 
        "count": 1, 
        "timeWindow": 30,
        "minRequestAmount": 1,
    ...
    {
        "resource": "feign#ali-user#GET#/users/detail",  // 当前服务对依赖服务降级
        "grade": 2, 
        "count": 1, 
        "timeWindow": 30,
        "minRequestAmount": 1,
